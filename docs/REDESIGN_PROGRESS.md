...
- 2026-09-13 — Email guest invitations (mail bridge) — Added external email guests to the event editor and guest list (wireframes `docs/wireframes/*` treated as guidelines). A plain email typed into the guest box that has no Nostr match surfaces an "Invite … by email" row (no new field; existing npub/NIP-05/name resolution unchanged). Email guests persist on the event as a new `["guest_email", address]` tag — inside the encrypted content for private events, a public tag for public ones — parsed into `ICalendarEvent.guestEmails`; readers unaware of the tag ignore it. Saving with email guests runs a new `invite-email-guests` publish step: one kind-1059 gift wrap to the mail bridge (kind-13 seal signed by the organizer, kind-1301 rumor carrying a full RFC 2822 invite as a byte string, guest addresses as `deliver` tags), mirroring the sister `nail` mailstr bridge protocol (docs/ARCHITECTURE.md §4–§5). The bridge is discovered over NIP-05 at `_smtp@<From domain>` (Settings override supported); the From must be an address the signing key owns, checked against its NIP-05 record exactly as the bridge authorizes on `seal.pubkey`. Alias suggestions come from the Formstr API only when `VITE_MAIL_API_BASE_URL` is configured — manual entry is always available, and the API is never authoritative. A gated "Get a mailstr address" notice shows when an email guest exists but the organizer has no known address. `docs/nostr-layer-reference.html` updated with the new tag and an "Email guest invitations (bridge)" flow. Typecheck, lint, focused `event-participants` (2) and new `email-guest` (2) E2E specs pass; full web E2E 55 passed / 1 pre-existing date-boundary navigation flake that passes in isolation.
