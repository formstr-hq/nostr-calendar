...
| F-DEVICE-CAL                  | done (Android verified; iOS needs macOS review) | 2026-08-01 | Implemented `docs/device-calendars-plan.md` in full: Android read+write EventKit-equivalent (`DeviceCalendarPlugin.java` + `WRITE_CALENDAR`), new iOS `DeviceCalendarPlugin.swift`/`IOSDeviceCalendarStore.swift`/`IOSRecurrenceRuleCodec.swift` (EventKit, RRULE↔`EKRecurrenceRule`), device-event edit/delete routed around `EditEventPage.tsx`'s pubkey gate via new `EditDeviceEventPage.tsx`, native-readable `cal:device_events` reminder snapshot consumed by both `NotificationWorker.java` and `IOSNotificationScheduler.swift`, and the sidebar/settings "Synced"/"Device only" grouped UI (mockups 24-28) via new `CollapsibleGroup`/`CalendarManageDialog` device variant/`DeviceCalendarStaticRow`. `DeviceCalendarsSection.tsx` deleted (dead, folded into the grouped list).                                                                                                                                                                                                                                                                                                                                          |

## Session log

<!-- newest first: date — phase — what was done — e2e status -->

- 2026-08-10 — Timed-event stacking containment — Isolated the day and week timeline stacking contexts so later overlapping events retain their local draw order without appearing above sticky headers or other app chrome while scrolling. Browser QA covered populated day and week views after scrolling; typecheck, lint, and diff checks pass.

- 2026-08-10 — Sticky all-day event rows — Made the all-day event sections remain visible while scrolling the day and week timelines. Week rows stack below the sticky date header, and mobile swipe clipping no longer creates a non-scrolling overflow ancestor that blocks sticky positioning. Browser QA covered desktop, mobile web, and the native iOS main scroll pane; typecheck, lint, diff checks, and focused navigation/mobile E2E (13 passed) are green.

- 2026-08-10 — Mobile week event title legibility — Reduced timed and all-day event-chip titles from 12.5px to 10px only in the mobile week view, allowed them to wrap between words, and replaced ellipsis truncation with clipping so narrow seven-column cells reveal more of each title without splitting words. Desktop week chips and mobile day/month chips retain their existing presentation. Typecheck, lint, and diff checks pass.

- 2026-09-13 — Email guest RSVPs (private events) — Email guests can now respond to a private event without a Nostr account. The organizer generates a stable **one-time keypair per email guest**; the guest's pubkey is written as the third element of the `guest_email` tag *inside the encrypted content* (`["guest_email", address, guestPubkey]`, parsed into `ICalendarEvent.guestPubkeys`), while the nsec stays local (`src/utils/emailGuestKeys.ts`) and rides only in that guest's invite link as an `#nkeys1…` fragment (`{ nsec, email }`, encoded/decoded via `src/utils/emailGuestLink.ts` using the SDK's `nkeys` TLV envelope). The secret is never put on the event because every participant shares the viewKey. A guest who opens their link adopts the identity in memory only (`src/stores/guestSession.ts`, never persisted), gets the RSVP bar instead of the login-to-add prompt, and publishes a normal private RSVP (32069) signed by that key (`publishPrivateRSVPEvent` now takes an optional signer; `buildAndSign` accepts a signer override). The host resolves the responder pubkey back to the address via `guestEmailByPubkey` and renders it with a "via email" label and status. Because a per-guest link is unique, guests with an RSVP key each get their own bridge wrap; guests without one still share a single wrap (`buildEmailInviteWraps`). Email guests are deliberately never added to `participants`/`p` tags — that would emit a `#p`-tagged gift wrap to their one-time key and leak the identity on relays. Public events remain invite-only. `docs/nostr-layer-reference.html` updated (tag third element, guest-RSVP field, wrap-splitting note). Typecheck, lint, focused `email-guest` (2) + new `email-guest-rsvp` (2) + `event-participants` (2) E2E specs pass.

- 2026-09-13 — Email guest invitations (mail bridge) — Added external email guests to the event editor and guest list (wireframes `docs/wireframes/*` treated as guidelines). A plain email typed into the guest box that has no Nostr match surfaces an "Invite … by email" row (no new field; existing npub/NIP-05/name resolution unchanged); the guest-box placeholder now reads "Search name, NIP-05, npub, or email". Email guests persist on the event as a new `["guest_email", address]` tag — inside the encrypted content for private events, a public tag for public ones — parsed into `ICalendarEvent.guestEmails`; readers unaware of the tag ignore it. Saving with email guests runs a new `invite-email-guests` publish step: one kind-1059 gift wrap to the mail bridge (kind-13 seal signed by the organizer, kind-1301 rumor carrying a full RFC 2822 invite as a byte string, guest addresses as `deliver` tags), mirroring the sister `nail` mailstr bridge protocol (docs/ARCHITECTURE.md §4–§5). The bridge is discovered over NIP-05 at `_smtp@<From domain>`; the From must be an address the signing key owns, checked against its NIP-05 record exactly as the bridge authorizes on `seal.pubkey`. Alias suggestions come from the Formstr account API (`api.formstr.app`, NIP-98, override via `VITE_MAIL_API_BASE_URL`) — a convenience only, since manual entry always works and the gated "Get a mailstr address" notice appears only after a definitive 404, not while loading. Warm email-guest styling resolves its light/dark palette via `theme.applyStyles("dark", …)` (`src/theme/mailStyles.ts`) rather than `theme.palette.mode`, which is static under this app's CSS-variable theming. `docs/nostr-layer-reference.html` updated with the new tag and an "Email guest invitations (bridge)" flow. Typecheck, lint, focused `event-participants` (2) and `email-guest` (2) E2E specs pass.

- 2026-08-10 — Dark-mode foreground and auth contrast — Removed all non-custom text foregrounds from `sx`, including nested Formstr selectors; calendar event titles are the explicit black/white exception and now choose their foreground from calendar-color luminance in both modes. Other calendar-derived text is retained only for colors that are not black or white. Converted selected sidebar and booking controls to MUI variants so component-level contrast remains intact, and made default-primary outlined buttons consume `text.primary` while preserving semantic colors such as error. Neutral primary accents now invert in dark mode at the color-scheme synchronization source, keeping contained and outlined controls distinguishable while leaving chromatic custom accents unchanged. Made ordinary calendar dates, the top-bar brand, and the month-sheet action inherit their surrounding foregrounds, and made the shared today marker invert foreground/surface colors naturally in each mode. Paired the portaled shared bottom-sheet surface with the active theme foreground, strengthened outlined-input borders in both color schemes, and gave the fallback profile avatar a distinct token-based surface. Typecheck and lint passed before the final inherited foreground adjustments; E2E was started with an incorrect filter that launched the full suite, then stopped at the user's request. Further testing is intentionally deferred to the user.

- 2026-08-05 — Sidebar booking-link overflow — Replaced the absolutely positioned booking-page actions with a fixed-width flex action group, constrained long titles to the remaining row width with an ellipsis, and added a full-title tooltip. Typecheck, focused ESLint, and diff checks pass.

- 2026-08-05 — Participant contact suggestions — Added a separate author-scoped NIP-02 kind-3 contact-list interest because local-relay's kind-0 NIP-50 content search neither returns nor searches contact <code>p</code> tags. Participant history, contacts, exact identities, and NIP-50 profiles now merge by lowercase pubkey. Empty suggestions stably promote named people; typed searches stably promote matching contacts/prior participants instead. Contact/history overlap renders one row with both a contact icon and an event icon next to the avatar. Removed the prior-participant avatar border and tooltip. Kind 3 is read-only, so there is no wire-format migration or compatibility window. Typecheck, lint, and diff checks passed before the ranking adjustment; focused E2E verification remains pending.

- 2026-08-04 — Event-update notification diagnostics — Removed the WebView foreground event-update comparison and local-notification flow; foreground relay updates still replace cached events and reconcile ordinary reminders, while native background pollers remain the sole producers of meeting-update alerts. Added step-by-step Android <code>EventUpdateWorker</code> debug logs for worker inputs, eligibility, query construction and relay results, replacement validation/decryption/comparison, persistence, widget/reminder work, notification decisions/posting, and completion. No wire format or migration behavior changed. Typecheck, lint, Android Java compilation, and diff checks pass. Full web E2E reached 46/58; 12 unrelated shared-relay/state and date-baseline failures span booking, calendar setup, event publishing, participant search, and navigation.

- 2026-08-02 — Private event-update polling reliability — Android and iOS background pollers now read the cached event snapshot once, select only future or recurring private events with view keys, and issue one NIP-01 <code>REQ</code> per relay containing exact kind/author OR filters with grouped d-tag lists instead of one query per event; public events are excluded from update alerts in native and foreground paths. Native NIP-44 parsing and foreground comparison now align across time, title, description, location, image, recurrence, categories, references, forms, notification preference, and participant additions. Accepted replacements are persisted before reminder reconciliation and update notifications. This changes no wire format or migration behavior. Typecheck, focused ESLint, Android Java compilation, UTC Android unit tests, and diff checks pass. Focused web E2E was blocked by stale shared-relay data (the serial edit flow completed but found five copies of its fixed-title event); iOS build validation still requires macOS/Xcode.

- 2026-08-02 — Existing-participant invitation fix — Private-event edits now require the caller's original participant list, and applying an RSVP time suggestion preserves it instead of re-sending invitations to every participant. The invitation inbox also reconciles active and buffered invitations against all calendar lists so events accepted through shared links cannot retain an "Add to Calendar" action. Typecheck, lint, diff checks, and focused RSVP E2E (3 passed) are green.
...
- 2026-08-01 — Native widget seven-day agenda — Extended Android and iOS widgets from a rolling five-day query capped before rendering to seven local calendar days (today plus six), retained chronological calendar filtering/recurrence behavior, and grouped future events beneath full weekday/date headings while leaving today's rows directly under the existing date header. Event rows now consistently lead with time or "All day"; adaptive capacity shows up to five rows and a <code>+N more</code> remainder. Android refreshes its capacity on resize. Production build (including lint/typecheck), Android Java/resource compilation, UTC Android unit suite, and <code>git diff --check</code> pass. Full web E2E reached 52/58; six unrelated shared-state/timing specs failed, with two of those six passing on a serial rerun. iOS compilation still requires macOS/Xcode.

- 2026-08-01 — Android widget configuration polish — Replaced the default pink AppCompat widget-configuration screen with the redesign token palette (warm canvas, white bordered cards, ink-black accent), rounded panel/cards/buttons, and a dedicated no-action-bar theme. The configuration panel now sits within system-bar/display-cutout safe insets on a black backdrop so content does not meet or clip against phone edges. Calendar choices use static custom black check controls with no selectable background or ripple and allow two-line labels. Android resource processing and Java compilation pass.

- 2026-08-01 — iOS calendar widget — Added an iOS 17+ WidgetKit extension with small/medium layouts, App Intent calendar selection, five-day recurrence-aware Nostr loading, cached device-calendar occurrences, all-day formatting, universal-link app launch, and 30-minute timelines. Added the shared `group.app.formstr.calendar` App Group and `IOSWidgetDataBridge` so Capacitor preference changes and native event-update polling reload the extension without launching the WebView; device event snapshots now retain calendar/all-day metadata and a new device-calendar metadata snapshot feeds configuration choices. The extension target is embedded in the app and the previously omitted existing Swift files are included in the app target. `pnpm build`, full web E2E (58 passed), Android Java compilation, XML lint, and `git diff --check` pass; Xcode/Swift compilation still requires macOS because those tools are unavailable in this Linux environment.

- 2026-08-01 — Participant search — Restored local-relay `setSearchRelays` support and configured Calendar's kind-0 NIP-50 searches for dedicated relays with ordinary-read fallback. The picker uses a 100 ms free-text/NIP-05 debounce while complete npub/hex identities resolve immediately; combines network results with per-account IndexedDB history derived from event hosts and participants; excludes self and already-selected participants; marks prior contacts with a black/white avatar border and row-level tooltip; and displays NIP-05 only after pubkey verification. Logout wipes both participant history and the relay event cache. The worker's local content matcher is case-insensitive AND-over-whitespace substrings only, not full NIP-50. No kind, tag, content, encryption, or publish shape changed, so no wire-format migration is required.

- 2026-08-01 — F-DEVICE-CAL — Implemented the full device-calendar plan (`docs/device-calendars-plan.md`). **Android**: `DeviceCalendarPlugin.java` gained `createEvent`/`updateEvent`/`deleteEvent`/`updateCalendarColor` (whole-series only, RFC5545 `DURATION` when `RRULE` is set, composite `instanceId:eventId` parsed via `parseEventId`), `WRITE_CALENDAR` added to `AndroidManifest.xml`, `NotificationWorker.java` merges a new `cal:device_events` snapshot into its existing reconciliation loop unchanged. **iOS** (new): `DeviceCalendarPlugin.swift` + `IOSDeviceCalendarStore.swift` (EventKit, always full access, `.futureEvents` span), `IOSRecurrenceRuleCodec.swift` isolates RRULE↔`EKRecurrenceRule` conversion, `IOSNotificationScheduler.swift` merges the same `cal:device_events` key, `Info.plist` gained `NSCalendarsFullAccessUsageDescription`/`NSCalendarsUsageDescription`, registered in `AppDelegate.swift`. **TS**: `deviceCalendar.ts` bridge extended with the write methods; `deviceCalendars.ts` store gained `colorOverrides`, `createDeviceEvent`/`updateDeviceEvent`/`deleteDeviceEvent`, `updateCalendarColor` (native write with app-side fallback override), and writes the native-readable snapshot via `setSecureItem` after every refresh/write; `deviceCalendarAdapter.ts` gained `calendarEventToDeviceFields`/`deviceEventStableId`. **Editor**: new `EditDeviceEventPage.tsx` (sibling of `EditEventPage.tsx`, its pubkey gate is untouched), new route `/event/edit/device/:eventId`, `useEventSave.ts` branches device saves around all Nostr publish machinery, new `DeleteDeviceEventDialog.tsx` replaces `EventActionsBar`'s old export-only device path with Edit/Delete, calendar picker locked to a new read-only `DeviceCalendarStaticRow` for device events, forms UI hidden. **UI** (mockups 24-28): `EventChip`/`EventChipsRow` gained a phone-icon device treatment; new `CollapsibleGroup` powers "Synced"/"Device only" grouping in `SidebarContent.tsx` and `CalendarsSettingsPage.tsx`; `CalendarManageDialog.tsx` gained a color-only device variant; `DeviceCalendarsSection.tsx` deleted (dead, folded into the grouped list). `pnpm typecheck`, `pnpm lint`, and `pnpm build` are clean. Android: `./gradlew compileDebugJavaWithJavac` and `assembleDebug` are clean; installed and launched on the `Medium_Phone_API_36.1` emulator with no crash and `WRITE_CALENDAR` confirmed declared (one pre-existing, unrelated `RecurrenceUtilsTest` DST-drift failure — not touched this session, confirmed via `git log`). iOS cannot be built/verified in this Linux environment (no Xcode/Swift toolchain) — implemented carefully against documented EventKit APIs; needs a macOS/simulator review pass. Device-calendar event creation (v1 out of scope per the plan) and a dedicated mobile-Switch treatment for mockup 28 (this session reused the existing checkbox pattern already used for Nostr calendars, for consistency) remain open follow-ups.

- 2026-08-01 — E2E booking horizon fix — Made public-booking test selection respect the page's 30-day maximum advance window instead of always jumping to the next calendar month. This removes a date-dependent failure when the next month's first day falls outside the exact horizon. `pnpm typecheck`, `pnpm lint`, focused booking/scheduling E2E (7 passed), and full web E2E (57 passed) are green.

- 2026-08-01 — iOS shell/input fixes — Native iOS now constrains the app shell to the visual viewport and makes only the content pane scroll, leaving the top bar and mobile tab bar fixed. Week headers stick within that pane, and the calendar's initial current-time positioning targets it instead of the document. Native iOS text-entry controls now use 16px text to prevent WebView auto-zoom; Android and browser layouts are unchanged. Added mobile regression coverage for the iOS shell and input size. `pnpm typecheck`, `pnpm lint`, focused mobile E2E, and the production build are green. Full web E2E: 54 passed; three booking tests time out waiting for an enabled available day, including in a serial rerun, outside these changed paths. Simulator/device verification requires macOS/Xcode.

- 2026-07-31 — Event updates (#161) — Added foreground comparison and native best-effort polling for cached future/recurring replaceable events. Newer events update the native cache before a separate `event_updates` notification is posted; participant removals alone are silent, additions and other changed attributes are reported, author/self-removal alerts are suppressed, and time changes immediately reconcile reminder schedules. Android refreshes the calendar widget and uses a 15-minute network-constrained WorkManager job; iOS runs the equivalent poller on launch/foreground/OS-granted background refresh and uses an `event_updates` notification thread. Both native pollers decrypt private payloads using their cached nsec view keys. `pnpm typecheck`, lint, Android Java compilation, and web E2E were run; iOS build validation requires macOS/Xcode.

- 2026-07-31 — Android platform cleanup — Removed the generic `isAndroidNative()` gate from the passive `ICSListener`; Android `MainActivity` remains the sole native producer of `icsFileReceived`, while the listener can now be exercised in web tests. The only remaining `isAndroidNative()` use is the Login flow's NIP-55 signer discovery/rendering, which must stay Android-gated because the external signer plugin rejects non-Android calls. Corrected the native-layer reference's stale notification-platform description and added an ICS bridge regression test. `pnpm typecheck`, `pnpm lint`, focused `ics.spec.ts`, and full `pnpm test:e2e` are green (56 passed).

- 2026-07-31 — iOS notifications — Added best-effort parity for Android's incoming notification polling. The iOS app now queries up to three relays concurrently for current NIP-59 invitation (`1059`/`k=1052`) and booking (`1059`/`k=1057`/`k=1058`) wraps, suppresses locally cached invitations and booking-origin invitation wraps, only reports approved booking responses, and uses the same cursor/notification copy/routes as Android. Polling runs on launch, foreground, and a requested 15-minute `BGAppRefreshTask`; iOS may defer or skip this task, so this is not guaranteed delivery and APNs remains the future reliable option. iOS intentionally does not poll legacy kinds `84`, `1052`, `1057`, or `1058`. Native notifications use Capacitor's `cap_extra.openRoute`, and the shared JS listener now routes both event reminders and incoming notification taps. `pnpm typecheck`, `pnpm lint`, and `pnpm test:e2e` (55 passed) are clean. iOS build/simulator validation must run on macOS because this Linux environment has neither Xcode/`swiftc` nor `plutil`.
...
- 2026-07-25 — F-CAL-MGMT — Replaced the empty Settings → Calendars
  ...
  calendar rows show their event color and open the existing create/edit
  dialog, with create, rename, notification preference, color, and delete
  actions wired to `stores/calendarLists.ts`. The sidebar remains the compact
  visibility surface and retains `calendar-row` /
  `calendar-visibility-checkbox` contracts. No Nostr change: kind 32123
  remains self-encrypted and visibility remains local-only. Hardened
  `CalendarManageDialog` so async save/delete operations cannot close early;
  deletion now shows a loader and "Deleting…" while its NIP-09 publish is in
  flight, and relay failures remain visible for retry. Added Settings-route
  create/color/rename/delete E2E coverage. `tsc --noEmit -p
  tsconfig.app.json` and `eslint . --quiet` clean; focused Playwright:
  **4 passed**.

- 2026-07-25 — F-NOTIF — Migrated calendar invitation outer wraps to NIP-59/NIP-17 kind `1059`, retaining the encrypted kind-14 rumor, `signing_nsec`, and public `k=1052` calendar classifier. The client and Android background worker now dual-read legacy kind `1052` wraps so existing pending invitations remain visible. Dismiss/report publishes exactly one NIP-09 deletion: the embedded ephemeral key gives a relay-valid deletion when available, while legacy no-key invitations use the active signer deletion request. Kind `84` is legacy-read-only and is no longer published anywhere. Confirmed the existing local-relay NIP-09 `e`-tag tombstone enforcement already filters deleted wraps before invitation delivery, so no competing cache was added. Restyled invitation cards to use theme tokens; the Report action remains explicitly warning-coloured. Updated `nips/NIP-52E.md` and recorded the same deletion pattern as notes for the later booking flow. Typecheck and targeted lint clean; `invitations.spec.ts`: **4 passed**.
