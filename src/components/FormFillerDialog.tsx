...
} from "../hooks/useFormSubmissionStatus";
...
function getResponseTags(tags: NostrEvent["tags"]) {
  return tags.filter((tag) => tag[0] === "response" && tag[1]);
}
...
async function responseRowsFromEvent(
  ...
  let responseTags = getResponseTags(response.tags);
  if (!responseTags.length && form?.pubkey) {
    try {
      const decryptedTags = await decryptFormResponse(response, form?.pubkey);
      if (decryptedTags) {
        responseTags = getResponseTags(JSON.parse(decryptedTags));
      }
    } catch {
      responseTags = [];
    }
...
function buildFormSx(theme: Theme) {
  return {
    ...
    // Form body: stack sections with clear separation
    "& .form-body": {
      display: "flex",
    ...
    // Intro section (form name is already in DialogTitle)
    ...
    // Content section: column layout, spacing controlled per element
    "& .form-section:not(.form-intro)": {
      display: "flex",
      flexDirection: "column",
    },

    // Option groups (radio/checkbox questions): space before each group
    ...
    // Option group question label — bold
    ...
    // Text-input question labels — bold, space before, tight gap to input below
    "& .form-section > label": {
      fontSize: "0.875rem",
      fontWeight: 700,
      color: theme.palette.text.primary,
    ...
    // Fieldset fallback (some SDK field types)
    "& form fieldset": {
      border: "none",
      padding: 0,
      margin: 0,
