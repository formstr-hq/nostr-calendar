 ...
 */
export function buildFormstrResponsesUrl(_form: IFormAttachment): string {
  // currently ignoring dynamic urls as keys are not available. feature will be added later on
  const base = `https://formstr.app/dashboard`;
  return base;
  // return `${base}?viewKey=${encodeURIComponent(form.viewKey)}`;
}
