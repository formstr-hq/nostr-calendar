...
test("blocked dates remove that day's slots from the public page", async ({
  ...
  const dayLabel = blocked.toLocaleDateString("en-US", { weekday: "short" });
  const monthLabel = blocked.toLocaleDateString("en-US", { month: "short" });
  const blockedColumn = bob
    .locator(".MuiPaper-root")
    .filter({ hasText: dayLabel })
    .filter({ hasText: `${monthLabel} ${blocked.getDate()}` });
