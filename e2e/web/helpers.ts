...
/**
 * Opens the sidebar drawer and navigates to the bookings page. Mounting the
 * sidebar is what starts the booking-request relay subscriptions, so this is
 * the realistic path to a live /bookings view.
 */
export async function openBookingsViaSidebar(page: Page): Promise<void> {
  // Booking-related pages render their own <Header/> in addition to the
  // app-level one; both AppBars are position:fixed so only the last one in
  // the DOM is actually clickable.
  await page.getByRole("button", { name: "open menu" }).last().click();
